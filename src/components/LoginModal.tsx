...
export default function LoginModal({
  ...
  const handleUsernameValidity = async (
    event: ChangeEvent<HTMLInputElement>
  ) => {
    const usernameInputValue = event.currentTarget.value;
    if (usernameInputValue && usernameInputValue.length >= 3) {
      const usernameExists = await (
        await fetch(`/api/checkusername/${usernameInputValue}`)
      ).json();
  ...
  return (
    <Modal
      ...
      className="grid grid-rows-1 overflow-y-auto"
    >
      <div className="grid auto-rows-min m-auto md:w-1/2 lg:w-1/3 p-6 lg:p-12 bg-white dark:bg-zinc-950 dark:text-white focus:outline-none">
        ...
        <Typography id="modal-modal-description" sx={{ mt: 2 }}>
          By continuing, you are setting up a Bellspace account and agree to our
        ...
        <div className="mt-10">
          ...
          >
            <div className="p-1 w-full flex justify-center text-base font-semibold text-stone-500 dark:text-stone-400">
        ...
        >
          <TextField
            variant="standard"
            label="Username"
            onChange={handleUsernameValidity}
          ...
          <FormControl variant="standard">
            ...
            />
            <FormHelperText error={passwordIncorrect}>
              {passwordHelperText}
            </FormHelperText>
          </FormControl>
          <Button
            ...
            fullWidth
            type="submit"
            className="rounded-full my-5 font-semibold normal-case"
          >
            Log In
          </Button>
        </form>
